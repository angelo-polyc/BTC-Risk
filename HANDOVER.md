...
12. **Phase 2 volume refactor: move top-level CSVs onto the persistent volume.** Today (2026-04-23) the Railway deploy persists `data/raw/`, `data/hypotheses/`, `data/final/`, and `paper_trading_log.csv` on the `/app/data` volume, but the top-level canonical CSVs (`master_daily_view_*.csv`, `hypothesis_*.csv`, `weights.csv`, `thresholds.csv`, `weight_history_*.csv`, `health_check.csv`, `shadow_state.csv`, `export_manifest.json`) are still written to cwd (`/app`, ephemeral). Consequence: every redeploy resets the API to the committed-to-git state until `run_on_startup: true` fires a fresh pipeline run (~3 min warm). Acceptable today but not ideal — the "proper" fix is an env-var split where `BTC_MODEL_ROOT` stays `/app` (code) and `BTC_DATA_DIR=/app/data` becomes the single source of truth for persisted state. Concretely: (a) `export_csvs.py` `--out` args in `run_all.sh` get prefixed with `$BTC_DATA_DIR/`; (b) `health_check.py` and `shadow_tracker.py` `--out` args in `runtime_config.yaml` get prefixed similarly; (c) `data_access.py` already reads `BTC_MODEL_DIR` → just switch that env var to point at `/app/data`; (d) verify no other writer hardcodes a top-level path. Scope: 3–5 files, maybe 30–60 min of careful editing + a test redeploy. Not urgent; `run_on_startup: true` is cheap enough that this can wait. When done, `run_on_startup` can be flipped back to `false` since the API will serve fresh volume-backed state immediately on deploy.
