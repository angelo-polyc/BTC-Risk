...
**Status:** Canonical **v14** shipped; operational comms runtime (Phases 1-4 of the six-phase infra plan) shipped on top. Production migrated from Replit to **Railway** on 2026-04-23. Live browser dashboard shipped on top of the REST API on 2026-04-23 (resolves open item #11). Model code and weights are unchanged since 2026-04-18.
...
> **v14 deployment-live note (2026-04-22, evening):** production deployment is live on Replit Reserved VM at `https://btc-risk.replit.app`. Single-port topology (`.replit` + `runner.sh` + `api_server.py` with MCP mounted at `/mcp`) verified end-to-end: REST `/today` matches canonical state exactly, MCP client round-trip returns 17 tools with correct data, host-allowlist enforcement passes. Claude Code wired via `claude mcp add --transport http btc-drawdown-model https://btc-risk.replit.app/mcp --header "Authorization: Bearer ..."`. First scheduled daily pipeline fires at 22:00 UTC 2026-04-22. Open follow-ups: dashboard (deferred, see open item #11), audit-log push-to-GitHub (deferred), first-run failure handling (watch for Velo cold-pull timeout).

> **v14 Railway-migration note (2026-04-23):** production migrated from Replit to Railway at `https://btc-risk-model.up.railway.app`. Same single-container topology (`runner.sh` runs `daily_pipeline.py` APScheduler in background + uvicorn foreground). New files: `Dockerfile` (python:3.12-slim + git/bash/tini + requirements.txt + COPY . .), `railway.toml` (single-service Dockerfile build, `bash runner.sh` start, `/docs` healthcheck, timeout 100s). Railway's multi-service primitive couldn't be used: (a) `railway.toml`'s `[[services]]` array doesn't work; (b) shared volumes between services are not supported. So the Replit-era single-container design is what ships. Persistent volume attached at `/app/data` holds the expensive state: `data/raw/*.parquet` (9 sources), `data/hypotheses/*.csv`, `data/final/*.csv`, and `paper_trading_log.csv`. Top-level canonical CSVs (`master_daily_view_*.csv`, `hypothesis_*.csv`, etc.) remain ephemeral at `/app` but regenerate in ~3 min warm; `run_on_startup: true` in `runtime_config.yaml` means every fresh deploy triggers a pipeline run on boot. `audit_log.py` gained a `--no-git` flag so `paper_trading_log.csv` can live on the volume instead of being committed. MCP wire-up on Railway: `claude mcp add --transport http btc-drawdown-model https://btc-risk-model.up.railway.app/mcp --header "Authorization: Bearer ..."`. First Railway deploy with fresh pipeline run produced regime=bear, ensemble 0.527, percentile 0.811, position 0.000 on 2026-04-23 — materially more defensive than the 2026-04-17 snapshot in the repo. Claimed ~25-min Velo cold pull in old docs is wrong in practice; warm runs are ~3 min.

> **Railway gotchas worth remembering (2026-04-23 session):** (1) `railway.toml` is single-service only — multi-service layouts silently fall back to Railpack autodetect and break. (2) `startCommand` is not run through a shell — `${PORT:-8000}` is passed as a literal string unless wrapped in `sh -c`. (3) Port must be `$PORT` (Railway-injected), not a hardcoded value; the proxy routes to the injected port. (4) Volume bind-mounts SHADOW the mount path — mounting at `/app` wipes out the image's code; use `/app/data` (a path that doesn't exist in the image). (5) `healthcheckTimeout = 10` is too short even when things work; 60–100 is safer. (6) Railway volumes are per-service, not shareable; architectural plans that rely on a shared volume between web + cron services are dead on arrival.

> **v14 browser-dashboard note (2026-04-23):** live dashboard shipped at `https://btc-risk-model.up.railway.app/dashboard`, mounted into the same `api_server.py` process that serves REST + MCP. Single-file `dashboard.html` (82 KB) — no build step, no framework. Gated behind HTTP Basic Auth (`BTC_DASHBOARD_PASSWORD`, separate from the bearer token so it can be rotated independently); the server injects `BTC_API_TOKEN` into the template at render time so the page JS can call the same REST endpoints the MCP clients use. Panels: today's-call hero with contribution breakdown, BTC price & position (dual-axis, regime-banded), ensemble percentile with threshold bands read live from `/thresholds` (so it reflects the v14 0.45/0.80 taper, not hardcoded), position-function mini chart, drivers table + flags from `/health`, AUC attribution heatmap, hypothesis composites over time (5 solid + ETH dashed), sub-signal risk heatmap, track record with Sharpe/total/MDD vs buy-and-hold over 1Y/3Y/ALL windows, data-freshness pipeline footer. Node dry-run of the perf calculation matches HANDOVER numbers exactly (1Y Sharpe 1.289 vs target 1.29; ALL Sharpe 1.242 vs target 1.24; ALL total +556% vs target +556%; ALL MDD −31.9% vs target −32%). All fetches use relative paths so the Replit→Railway URL change is transparent to the client. Resolves open item #11.

> **v14 manifest note (2026-04-23):** `export_csvs.py` now writes a `canonical_start` field into `export_manifest.json`, sourced from `common.ENSEMBLE_FIT_START` (= 2021-06-30). The browser dashboard reads this field to scope its track-record perf calculation to the canonical fit window; otherwise the pre-2021 pre-calibration strategy returns baked into `master_daily_view_wf365.csv` (from 2015-09 onward) would inflate the "ALL" metrics. Falls back to the hardcoded `"2021-06-30"` in the dashboard JS if `/manifest` lacks the field — so the dashboard reads correctly on day 1 regardless of deploy order. Live manifest picks up the field after the next pipeline run (on Railway, that's the `run_on_startup: true` boot run).
...
This installs deps, pulls all 9 data sources (including Artemis), regenerates all 4 canonical variants, and exports the master CSVs + standalone hypothesis CSVs + manifest. Credentials are baked into `pull_all_raw_data.py` (see `credentials__1_.md`); `ARTEMIS_API_KEY` env var takes priority over the baked-in fallback.
...
3. **Operational comms runtime.** Phases 1-4 of the six-phase plan shipped in the 2026-04-22 ops session. `daily_pipeline.py` (APScheduler; chains `run_all.sh`, `health_check.py`, `shadow_tracker.py`, `audit_log.py` on a configurable cron; rotating logs, step timeouts). `audit_log.py` (appends today's row to git-tracked `paper_trading_log.csv`; structured commit messages; idempotent; starts empty on deploy — no backfill; optional `--push-best-effort` for remote replication). `api_server.py` (FastAPI REST with bearer auth, 17 read-only endpoints exposing the full CSV history from day 1). `mcp_server.py` (FastMCP over Streamable HTTP with the same bearer and same 17-tool surface for Claude Code agents) — **mounted into `api_server.py` at `/mcp` in production** so REST and MCP share one port, one URL, one bearer token. Shared loader in `data_access.py`. Runbook in `OPERATIONS.md`. **Production deployment:** single Replit Reserved VM Deployment running `runner.sh` (scheduler in background + combined API/MCP in foreground); config in `.replit`. **Remaining:** Phase 5 (Telegram alerts + trigger evaluator + dedup state, per `refit_report_v7.md` §4), and integration of `make_daily_chart.py` into the daily distribution. Not blockers for using the stack today.
...
11. **Live browser dashboard. RESOLVED in 2026-04-23.** Shipped as `dashboard.html` served at `/dashboard` by `api_server.py` with HTTP Basic Auth (`BTC_DASHBOARD_PASSWORD`, set in Railway Service Variables alongside `BTC_API_TOKEN`) and server-side token injection. Live at `https://btc-risk-model.up.railway.app/dashboard`. Single file, no build step, uses the REST endpoints already exposed by `api_server.py` + one new field (`canonical_start`) added to `export_manifest.json`. Fifteen endpoints feed thirteen panels; Node dry-run of the perf calculation matched HANDOVER targets exactly. The old static `btc_model_dashboard.html` mock (921 KB with baked-in data) can be retired at your discretion; `make_daily_chart.py` (matplotlib PNG) is unaffected and still in the repo for offline use.

12. **Phase 2 volume refactor: move top-level CSVs onto the persistent volume. RESOLVED in 2026-04-27.** Top-level CSVs (`master_daily_view_*.csv`, `hypothesis_*.csv`, `weights.csv`, `health_check.csv`, `shadow_state.csv`, `export_manifest.json`, `data_inventory.csv`, `raw_data_export.csv`) now write to `/app/data` via the `BTC_DATA_DIR` env var (defaulted to `/app/data` in `runner.sh`). API readers continue reading via `BTC_MODEL_DIR`, also defaulted to `/app/data`. Committed-in-git static reference files (`thresholds.csv`, `weight_history_wf365_y_{60,30}.csv`, `pinning_audit_findings.csv`, `health_check_history{,_extended}.csv`) are bootstrapped from `/app` to `/app/data` on container boot via a small idempotent loop in `runner.sh`. `run_on_startup: true` is no longer required for the API to be useful on a fresh deploy — kept for now as a belt-and-suspenders refresh-on-restart, can be flipped to `false` whenever you want.

13. **Daily fetcher cache-skip + freshness gate. RESOLVED in 2026-04-27.** Pre-fix, `pull_all_raw_data.py` short-circuited every source if a parquet existed at the target path (`if out_path.exists() and not force: SKIP (cached)` — 23 hits in the file). The daily cron called the script without `--force`, so once the volume had parquets they never refreshed; the model output looked alive (latest_date crept forward via rolling/ranked sub-signals propagating off stale tails) while inputs froze on whatever week they were last force-pulled. Compounded by `run_all.sh` piping `pull_all_raw_data.py` to `grep | head`, which masked python's exit code with grep's — pull failures were silently logged as success. Fix: `--force` added to `run_all.sh`, `set -o pipefail` added so future pull failures actually fail the chain, and a new `check_freshness 36` step inserted in `runtime_config.yaml` between `run_all` and `health_check` that aborts the chain if no parquet is younger than 36h. New `data_freshness` section in `export_manifest.json` (newest/oldest age hours, per-source breakdown) so `/manifest` actually shows freshness instead of just `generated_at`. The original 2026-04-23→27 stuck-model incident was traced to this exact failure mode — see commit `f82d6e2` and the v14 forensics in `refit_report` for the full story.

14. **`data_inventory.csv` and `raw_data_export.csv` regenerated each pipeline run. RESOLVED in 2026-04-27.** Both files were committed to git on 2026-04-22 and never rewritten by the pipeline — `git grep` confirmed no writer in the codebase. `/data_inventory` and `/raw` (and `/raw/columns`) had been serving the v14-ship snapshot regardless of what the parquets actually contained, which misled the original 2026-04-27 cache-skip diagnosis. Fix: `regenerate_data_inventory()` and `regenerate_raw_data_export()` added to `export_csvs.py`, called from `main()` after the master/weights/hypothesis exports. `data_inventory` walks `data/raw/**/*.parquet` for per-source coverage; `raw_data_export` produces a 162-column wide table keyed by date, pivoting long-format parquets (Velo, Coinglass H2 liquidations) on `exchange`. Schema (group/series/column names) preserved bit-for-bit via override maps — `cg_cycle/cg_h2/cg_h3` short-form groups in inventory, `TFF_133741`/`BTC`/`ETH` series mappings in inventory, `cftc__TFF_133741__*` column prefixes in raw_export — so any consumer filtering on the historical names keeps working. Defensive: per-parquet errors get logged and skipped, top-level merge failure logs traceback and returns cleanly (so the broader chain isn't held hostage to a single bad parquet).
...
e3373f705da6a7e1  export_csvs.py                    (v14 dashboard: canonical_start field in manifest)
...
046f2df0289710a4  api_server.py                     (v14 dashboard: /dashboard route + Basic Auth)
405f16bda3e7e634  mcp_server.py                     (new in v14 ops)
fa9fabc8b1c1ca93  requirements-ops.txt              (new in v14 ops)
7df103e8d8832316  OPERATIONS.md                     (v14 dashboard: /dashboard section + BTC_DASHBOARD_PASSWORD + Railway URL)
92932a344cc34358  dashboard.html                    (new in v14 dashboard)
```
